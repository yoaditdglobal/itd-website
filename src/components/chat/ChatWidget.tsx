...
export default function ChatWidget() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (data.leadDone) setLeadDone(true);
      }
    } catch {
      /* ignore */
    }
  ...
  // ── Persist conversation + funnel progress ──
  useEffect(() => {
    try {
      localStorage.setItem(
        STORE_KEY,
        JSON.stringify({ sessionId: sessionId.current, messages, answers, flow, leadDone }),
      );
    } catch {
      /* ignore */
    }
    ...
    setMessages((prev) =>
      prev.length === 0
        ? [
            ...
            { id: newId(), role: "assistant", content: "", card: { kind: "entry" } },
          ]
        : prev,
    );
    track("chat_open");
